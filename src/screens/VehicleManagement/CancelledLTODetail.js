...
const CancelledLTODetail = () => {
  const dispatch = useDispatch();
  const route = useRoute();
  const { itemData } = route.params;
  const [detailData, setDetailData] = useState(null);
  const allCancelledDetails = useSelector((state) => state.lto.cancelledLTO);
  ...
  useEffect(() => {
    if (!allCancelledDetails.length) {
      dispatch(getCancelledLTO());
    } else {
      const itemDetail = allCancelledDetails.find(
        (item) => item.idx === itemData.id,
      );
      setDetailData(itemDetail);
    }
  }, [allCancelledDetails, itemData.ltoId, dispatch]);
...
export default CancelledLTODetail;
