...
const AllLTO = () => {
  ...
  const { roleId } = user || {};
  const { LTO, loading, error } = useSelector((state) => state.lto);
  const [searchText, setSearchText] = useState('');

  // Fetch LTO data from API
  useEffect(() => {
    if (roleId) {
      dispatch(getLTO());
    }
  ...
  useEffect(() => {
    const mappedData = LTO?.filter(
      (item) => item.isdeleted === 0 || item.isdeleted == null,
    ).map((item) => ({
      id: item.idx,
  ...
  const handlePress = (item) => {
    navigation.navigate('ApprovedLTODetail', { itemData: item }); // Navigate to ApprovedLTO screen with LTO data
  };
            ...
            >
              {filteredResults.length > 0 ? (
                filteredResults.map((item) => (
                  <Card key={item.id} item={item} onPress={handlePress} />
                ))
              ) : (
                <Text
                  style={{
